          ...
          {feed.map((value, index) => {
            if(value.class === CLASS_TEXT_COMMENT) {
              if(value.format === FORMAT_TEXT) {
                return <TextComment key={value.uuid} data={value} seek={props.seek} />;
              } else if(value.format === FORMAT_ABC_NOTATION) {
                return <ABCNotationDisplay key={value.uuid} data={value} seek={props.seek} />;
              }
            } else if(value.class === CLASS_VOICE_RECORDING) {
              return <VoiceRecording key={value.uuid} data={value} seek={props.seek} />
            }
            return <></>;
          })}
        </div>
